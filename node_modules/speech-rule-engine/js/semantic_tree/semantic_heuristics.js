...
import { SemanticTreeHeuristic, SemanticMultiHeuristic } from './semantic_heuristic.js';
...
function combinedNodes(nodes, role) {
    let node = SemanticHeuristics.factory.makeBranchNode(SemanticType.PUNCTUATION, nodes, []);
    node.role = role;
    return node;
}
SemanticHeuristics.add(new SemanticMultiHeuristic('op_with_limits', (nodes) => {
    ...
    return nodes[0].type === SemanticType.OPERATOR &&
        nodes.slice(1).some(node => node.type === SemanticType.RELSEQ ||
