...
class SemanticProcessor {
    ...
    wrapPostfix(split) {
        ...
        if (((_a = split.div) === null || _a === void 0 ? void 0 : _a.role) === semantic_meaning_js_1.SemanticRole.POSTFIXOP) {
            if (!split.tail.length || split.tail[0].type === semantic_meaning_js_1.SemanticType.OPERATOR) {
                split.head = [SemanticProcessor.getInstance().postfixNode_(SemanticProcessor.getInstance().implicitNode(split.head), [split.div])];
    ...
    punctuatedNode_(nodes, punctuations) {
        ...
        if (SemanticPred.singlePunctAtPosition(nodes, punctuations, 0)) {
            newNode.role = (fpunct.childNodes.length && !fpunct.embellished) ?
                fpunct.role : semantic_meaning_js_1.SemanticRole.STARTPUNCT;
        }
        else if (SemanticPred.singlePunctAtPosition(nodes, punctuations, nodes.length - 1)) {
            newNode.role = (fpunct.childNodes.length && !fpunct.embellished) ?
                fpunct.role : semantic_meaning_js_1.SemanticRole.ENDPUNCT;
        }
    ...
    getIntegralArgs_(nodes, args = []) {
        ...
        if (SemanticPred.isGeneralFunctionBoundary(firstNode)) {
            let { integrand: args2, rest: rest2 } = SemanticProcessor.getInstance().getIntegralArgs_(args);
...
SemanticProcessor.MML_TO_BOUNDS_ = {
    ...
    [semantic_util_js_1.MMLTAGS.MSUP]: { type: semantic_meaning_js_1.SemanticType.SUPERSCRIPT, length: 1, accent: false },
    [semantic_util_js_1.MMLTAGS.MSUBSUP]: { type: semantic_meaning_js_1.SemanticType.SUBSCRIPT, length: 2, accent: false },
    [semantic_util_js_1.MMLTAGS.MUNDER]: { type: semantic_meaning_js_1.SemanticType.UNDERSCORE, length: 1, accent: true },
    [semantic_util_js_1.MMLTAGS.MOVER]: { type: semantic_meaning_js_1.SemanticType.OVERSCORE, length: 1, accent: true },
    [semantic_util_js_1.MMLTAGS.MUNDEROVER]: { type: semantic_meaning_js_1.SemanticType.UNDERSCORE, length: 2, accent: true }
};
